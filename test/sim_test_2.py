...
inital_angles = [60,60,60,0]

def line_data(vertical_angle, horizontal_angle, length, orgin):
    x_val = np.cos(np.radians(horizontal_angle))*length + orgin[0]
...
def find_angles(x,y,z):
    ...
    angle_3 = 270
    angle_4 = np.arctan(y/x)
    if np.radians(angle_2) >= 360:
        angle_2 -= 2*np.pi
    return [np.degrees(angle_1+angle_1_addition),np.degrees(angle_2),angle_3, np.degrees(angle_4)]
...
def path_to(inital, final):
    ...
    for w in range(0, 4):
        ...
        if (abs(final[w]-inital[w]) > 180):
            ...
            steps = int(negative_max + min(couple))
            ...
            steps = int(max(couple) - min(couple))
            if inital[w] == max(couple):
                for x in range(0, steps):
                    path[w].append(inital[w]-x)
            elif final[w] == max(couple):
...
def end_effector_position(angles):
    z = 0
    x = 0
    h = values['arm_lengths_milimeters'][0]*np.cos(np.radians(angles[0])) + values['arm_lengths_milimeters'][1]*np.cos(np.radians(angles[1]))
    for i in range(0,3):
        z += np.sin(np.radians(angles[i]))*values['arm_lengths_milimeters'][i]
        # print(z)
    x = np.cos(np.radians(angles[3]))*h
        # print(x)
    y = np.sin(np.radians(angles[3]))*h
    return [x, y, z]

test = find_angles(225,100,100)
