...
class Path(object):
  ...
  def find_path_to(self, goal):
      final = self.inverse_kinematics(goal)
      self.path, theta = self.animation_path(self.values['iA'],final)
      self.steper_path(theta)
      return self.path, self.step_path
...
class Path_Exe(object):
    def __init__(self):
        ...
        self.path = Path([0,0,0,0.0],[0.20, 0.150, 0.050])
    ...
    def path_exe(self, travel, overhead_poll, overhead_push, hand_poll, hand_push, sim_pull, sim_push):
        ...
        path_finder = Path()
        path, step_path = path_finder.find_path_to(xyz)
        plotter = Plot()
        for x in range(0,len(path[0])):
            sim_push.put(plotter.postion([path[0][x], path[1][x], path[2][x], path[3][x]]))
        print("posted path")
